...
t('многостраничная версия: 6 страниц', () => {
  const files = fs.readdirSync(SITE).filter(f => f.endsWith('.html') && f !== '404.html').sort();
  assert(files.length === 6, 'файлов: ' + files.join(','));
  assert(fs.existsSync(path.join(SITE, '404.html')), 'нет страницы 404');
});
t('на каждой странице есть телефон, часы и меню-навигация', () => {
  for (const f of fs.readdirSync(SITE).filter(x => x.endsWith('.html') && x !== '404.html')) {
...
window.addEventListener('load', () => setTimeout(() => {
  ...
  t('оформление: валидация, оплата, экран успеха с отправкой на телефон', () => {
    ...
    click($('#toStep2'));
    assert(/Подтвердить заказ/.test($('#payBtn').textContent), 'шаг оплаты не открылся: ' + $('#payBtn').textContent);
    // БЕЗОПАСНОСТЬ: пока ЮKassa не подключена, полей ввода карты быть не должно
    assert(!$('#inCard'), 'на сайте есть поле ввода номера карты без реального платёжного шлюза!');
    assert($('#payCards .rc[data-pay="cash"]'), 'нет варианта «оплата при получении»');
    assert(!$('#payCards .rc[data-pay="card"]') || !!$('#inCard'), 'онлайн-оплата показана, но не подключена');
  ...
  setTimeout(() => {
    t('после заказа: экран успеха, WhatsApp и SMS на номер кафе', () => {
      ...
      assert(!/оплачено картой/.test(html), 'сайт пишет «оплачено», хотя онлайн-оплата не подключена');
      assert(/при получении|СБП/.test(html), 'нет честного указания способа оплаты');
    });
