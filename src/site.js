...
const FAQ = [
  ...
  { q:'Как можно оплатить?', a:'Оплата при получении: наличными или картой курьеру. Также можно перевести по СБП. Онлайн-оплата картой на сайте (ЮKassa) — в процессе подключения.' },
  ...
  if (checkoutStep === 1) {
    ...
    b.innerHTML = `
      ...
      <div class="radio-cards" id="payCards">
        <label class="rc on" data-pay="cash"><span class="rc-dot"></span><span><b>${mode === 'pickup' ? 'Оплата при получении' : 'Оплата курьеру при получении'}</b><span>Наличными или картой при получении</span></span><span class="rc-price">💵</span></label>
        <label class="rc" data-pay="sbp"><span class="rc-dot"></span><span><b>Перевод по СБП</b><span>Перевод на телефон ${esc(SETTINGS.phone)} после подтверждения заказа</span></span><span class="rc-price">📱</span></label>
        ${PAY.provider === 'yookassa' ? `<label class="rc" data-pay="card"><span class="rc-dot"></span><span><b>Картой онлайн · ЮKassa</b><span>Безопасная оплата, чек придёт на телефон</span></span><span class="rc-price">💳</span></label>` : ''}
      </div>
      ${PAY.provider === 'yookassa' ? `<div id="cardFields" style="margin-top:18px">
      ...
      </div>` : `<div class="pay-note">💡 Оплату картой онлайн пока не подключили (ЮKassa в процессе). Оплатите при получении или переводом по СБП — администратор подтвердит заказ по телефону.</div>`}
      <div style="display:flex;gap:12px;margin-top:22px">
        ...
        <button class="btn btn-primary" id="payBtn" style="flex:1">${(t.total === 0 || PAY.provider !== 'yookassa') ? 'Подтвердить заказ' : 'Оплатить ' + rub(t.total)}</button>
      </div>`;
    $$('#payCards .rc').forEach(c => c.addEventListener('click', () => {
      ...
      const cf = $('#cardFields');
      if (cf) {
        cf.style.display = (c.dataset.pay === 'cash' || c.dataset.pay === 'sbp') ? 'none' : 'block';
        $$('#cardFields input').forEach(i => i.disabled = c.dataset.pay !== 'card');
      }
      $('#payBtn').textContent = (c.dataset.pay === 'card' && PAY.provider === 'yookassa')
        ? 'Оплатить ' + rub(cartTotals().total) : 'Подтвердить заказ';
    }));
    if ($('#inCard')) { maskCard($('#inCard')); maskExp($('#inExp')); }
...
function orderText(o) {
  return `🟠 НОВЫЙ ЗАКАЗ ${o.num} — кафе «${SETTINGS.brand}», ${SETTINGS.city}\n`
    ...
    + `\nИмя: ${o.name}\nТелефон: ${o.phone}\nКогда: ${o.when}`
    + `${o.note ? '\nКомментарий: ' + o.note : ''}`
    + `\nОплата: ${({ card: (PAY.provider === 'yookassa' ? 'картой онлайн (ЮKassa)' : 'при получении'), sbp: 'перевод по СБП', cash: 'при получении' })[o.pay] || o.pay}`
    + (PAY.provider === 'yookassa' ? '' : '\n⚠ Оплата онлайн не подключена — принять оплату при получении');
}
...
function showSuccess() {
  ...
  const payNames = {
    card: PAY.provider === 'yookassa' ? 'оплачено картой онлайн' : 'оплата при получении',
    sbp: 'перевод по СБП — администратор подтвердит заказ',
...
const LEGAL = {
  ...
  terms: `<h3>Условия заказа и доставки</h3>
    ...
    <h4>3. Оплата</h4><p>При получении (наличными или картой курьеру) либо переводом по СБП. Онлайн-оплата картой — после подключения ЮKassa.</p>
